...
const TeacherLoginForm = ({ onSuccess, setUser }) => {
  ...
  const handleTeacherLogin = async (e) => {
    ...
    try {
      const res = await axios.post('/api/teachers/login', formData);
...
const TeacherSignupForm = ({ onSuccess }) => {
  ...
  const handleTeacherSignup = async (e) => {
    ...
    try {
      const res = await axios.post('/api/teachers/signup', formData);
      if (res.data.ok) {
      ...
      if (err.response?.data?.msg === 'Teacher already exists') {
        try {
          const loginRes = await axios.post('/api/auth/teacher/login', {
            email: formData.email,
            password: formData.password,
          });
